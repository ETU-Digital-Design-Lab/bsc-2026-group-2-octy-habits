...
OctyInsight buildOctyInsight({
  ...
  final loadPenalty = totalHabits > 6
      ? ((totalHabits - 6) * 0.03).clamp(0.0, 0.15)
      : 0.0;
  final dailyPenalty = totalHabits <= 0
      ? 0.0
      : ((totalHabits - doneTodayCount) / totalHabits) * 0.15;
  ...
  final doneTodayRatio = totalHabits <= 0
      ? 0.0
      : (doneTodayCount / totalHabits).clamp(0.0, 1.0);
