...
final octyInsightProvider = Provider<OctyInsight>((ref) {
  ...
  final weeklyCounts =
      ref.watch(weeklyDoneCountsProvider).valueOrNull ?? const <String, int>{};
  final events =
      ref.watch(recentAppEventsProvider).valueOrNull ?? const <AppEventEntry>[];
  ...
  final eveningEvents = events
      .where((e) => e.hour >= 18 && e.hour <= 23)
      .length;
